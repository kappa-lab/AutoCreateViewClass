...
public static class AutoCreateViewClass
{
    ...
    private static void Create(string base_class_name)
    {
        ...
        Text[] wi = target.GetComponentsInChildren<Text>(true).Where(i => i.gameObject.name.Contains("Txt")).ToArray();
        for (int i = 0; i < wi.Length; i++)
        {
            sb.AppendLine("\t\t[SerializeField] private " + wi[i].GetType().ToString().Replace("UnityEngine.UI.","") + " _" + wi[i].gameObject.name + ";");
        }
        Image[] pa = target.GetComponentsInChildren<Image>(true).Where(i => i.gameObject.name.Contains("Img")).ToArray();;
        for (int i = 0; i < pa.Length; i++)
        {
            sb.AppendLine("\t\t[SerializeField] private " + pa[i].GetType().ToString().Replace("UnityEngine.UI.", "") + " _" + pa[i].gameObject.name + ";");
        }
        Button[] bt = target.GetComponentsInChildren<Button>(true).Where(i => i.gameObject.name.Contains("Btn")).ToArray(); ;
        for (int i = 0; i < bt.Length; i++)
        {
            sb.AppendLine("\t\t[SerializeField] private " + bt[i].GetType().ToString().Replace("UnityEngine.UI.", "") + " _" + bt[i].gameObject.name + ";");
        }
